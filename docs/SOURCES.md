...
|---|---|---|---|---|---|
| 1 | **Sleeper — league state** | `api.sleeper.app/v1` (state, league, rosters, users, matchups, transactions, traded picks) | roster identity, scoring settings, slot layout, standings, waiver budget; `state` also resolves the CURRENT NFL week for the GUI's auto-load (`nfl_state`, `roster_source: sleeper` only); a roster's `starters`/`reserve` arrays, zipped against the league's own ordered slot layout, give the live LINEUP baseline (`slots_source`, replacing `weekly/lineup_state.yml`); `rosters`+`users` also join into every OTHER team's roster for the free-agent exclusion set (`league_rosters_source: sleeper`, fetched fresh every run — see row 5 below and section B's "Rival rosters"); `matchups` also resolves this week's live head-to-head OPPONENT'S actual started lineup (`fetch_opponent_starters`), joined against row 5's players dump into `LoadedReport.opponent_starters` — feeds `week.opponent_overlap`'s same-team-stacking discount / DEF-leverage bonus in `ffbot.gameplan`; fetched only when `season.opponent_correlation_weight` is nonzero | `sleeper:` block; `season.opponent_correlation_weight` gates the matchups-for-starters read | yes |
