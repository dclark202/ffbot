...
- **`policy.can_drop`/`policy.can_bid_on`'s safety guardrails apply at every level, identically.** Undroppable flags, the never-drop list, draft-round/ownership protection, and FAAB/rolling-priority caps are facts about the transaction, not strategy — they were never part of the spice ladder and B7 didn't change that.
