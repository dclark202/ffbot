...
rolling-waiver processing -- see league.yml's waiver_type). Each trigger
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    ...
    p.add_argument("--refresh-league-rosters", action="store_true", help="also refresh league_rosters.yml via import_league_rosters.py --live before a fired run (needs sleeper.league_id)")
    ...
    p.add_argument("--stream", nargs="*", default=None, metavar="POS")
    p.add_argument("--waivers", action="store_true")
    p.add_argument("--faab", type=int, default=None)
...
def _fire(trigger: Trigger, args: argparse.Namespace, season: int, week_num: int) -> bool:
    """Run week_report.py's exact pipeline for `trigger`, writing
    reports/YYYY-wNN-<trigger-suffix>.md. Returns True on success; a
    failure prints a warning and returns False so the caller does NOT mark
    the trigger fired -- an unattended run that failed should be retried
    on the next poll, within the trigger's own grace window, not silently
    treated as done.
    """
    reports_dir = Path(args.reports_dir)
    trigger_suffix = trigger.id
    ...
    if args.stream:
        week_report_argv += ["--stream", *args.stream]
    if args.waivers:
        week_report_argv.append("--waivers")
    if args.faab is not None:
        week_report_argv += ["--faab", str(args.faab)]
    ...
    try:
        rc = week_report.main(week_report_argv)
...
def main(argv: list[str] | None = None) -> int:
    ...
    for trigger in sorted(due, key=lambda t: t.due_at):
        if _fire(trigger, args, season, week_num):
